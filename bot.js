...
bot.command("lyrics", async (ctx) => {
    ...
    if (songTitle) {
        try {
            ...
            ctx.reply(
                `There was an error searching for the song "${songTitle}".`,
            );
        }
    } else {
...
bot.command("download", async (ctx) => {
    ...
    if (!query) {
        return ctx.reply(
            "Please provide a song name. Usage: /download <song name>",
        );
    }
    ...
    try {
        ...
        if (!videos || videos.length === 0) {
            return ctx.reply(
                "No results found for your search. Please try a different song name.",
            );
        }

        // Limit to maximum 10 buttons (Telegram inline keyboard limit per row is fine, but total is safe)
        const inlineKeyboard = videos.slice(0, 10).map((video) => [
            Markup.button.callback(
                video.title, // Song title
                `download_${video.id}`, // Use video.id (this is the YouTube video ID)
            ),
        ]);

        console.log(
            `✅ Found ${videos.length} videos, showing ${inlineKeyboard.length} buttons for query: "${query}"`,
        );

        await ctx.reply(
            "🎵 Please choose a song to download:",
        ...
        // Optional: Send a more user-friendly message
        await ctx.reply(
            "Sorry, something went wrong while searching YouTube. Please try again later.",
        );
    }
});
...
bot.action(/^download_(.+)/, async (ctx) => {
    ...
    try {
        ...
        const completeAudio = await ctx.reply(
            `Download complete, sending audio: ${videoTitle}...`,
        );
...
bot.command("downloadvideo", async (ctx) => {
    ...
    if (!query) {
        return ctx.reply(
            "Please provide a video name or URL. Usage: /downloadvideo <video name>",
        );
    }

    try {
        // Search YouTube (returns array of video objects directly)
        const videos = await yt.search(query);
        ...
        const inlineKeyboard = videos.slice(0, 10).map((video) => {
            return [
                Markup.button.callback(
                    ...
                    `download_${video.id}`, // video.id is the videoId
                ),
            ];
        });

        console.log(
            "Sending inline keyboard with",
...
bot.action(/^downloadvideo_(.+)/, async (ctx) => {
    ...
    try {
        ...
        ctx.telegram.deleteMessage(
            ctx.chat.id,
            completeVideoMessage.message_id,
        );
...
bot.command("trending_music", async (ctx) => {
    try {
        ...
        ctx.reply(
            "Sorry, I couldn't fetch the trending music right now. Please try again later.",
        );
    }
});
